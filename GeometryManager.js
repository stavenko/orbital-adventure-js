...
export function getOrCreateGeometry(props) {
  if (!props.id) {
    console.warn('Cannot rely on props id');
  }
  const cached = PropsCache[props.id] || {};
...
export function createGeometry(props) {
  const geometry = new props.type(...props.arguments);
...
export function replaceGeometryIfNeeded(props, oldGeometry) {
  ...
  const cached = GeometryCache[oldGeometry.uuid] || {};
  if (props.type == cached.type && isEqual(props.arguments, cached.arguments)) {
    return oldGeometry;
  } else {
...
export function createBufferGeometry(props) {
  ...
  for (const key in props) {
    ...
    const attribute = new BufferAttribute(attr.array, attr.size);
    if (key == 'index') {
