...
export class PathEditor extends CanvasBase{
  ...
  getControlPoints(){
    return this.path.getNodes().map((node,i)=>{
      ...
      return {type: Mesh,
        ...
        onEnter:()=>{ this.setState({currentNodeHovered: i})},
        onLeave:()=>{ this.setState({currentNodeHovered: null})},
        onDrag: (evt)=>{ this.movePath(evt, node); }
  ...
  renderScene(){
    let lines = this.path.getRawGeometry().map(g=>{
      return {
        type: Line,
        geometry: {position:g},
